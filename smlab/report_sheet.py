...
def draw_report_sheet(surf: pygame.Surface, fonts: dict, event, timeline: EnergyTimeline) -> pygame.Rect:
    ...
    shadow(surf, panel)
    pygame.draw.rect(surf, SURFACE, panel, border_radius=18)
    pygame.draw.rect(surf, BORDER_STRONG, panel, 1, border_radius=18)
    ...
    pygame.draw.rect(surf, SURFACE_2, close, border_radius=9)
    pygame.draw.rect(surf, BORDER_STRONG, close, 1, border_radius=9)
    label = fonts["ui"].render("Close", True, TEXT)
    surf.blit(label, label.get_rect(center=close.center))

    headline = _headline(event)
    surf.blit(fonts["ui"].render(headline, True, ACCENT), (panel.x + 28, panel.y + 52))
...
def _produced(surf, fonts, rect, event, timeline: EnergyTimeline) -> None:
    surf.blit(fonts["section"].render("WHAT WAS PRODUCED", True, ACCENT), (rect.x, rect.y))
    y = rect.y + 24
    if getattr(event, "engine", "") == "pythia":
        # A full event has hundreds of particles; list the most energetic final ones.
        rows = sorted(event.finals(), key=lambda particle: -particle.p4.e)
    else:
    ...
    for particle in rows[:8]:
        ...
        surf.blit(detail, (rect.x + 16, y + 20))
        y += 42
        if y > rect.bottom - 90:
            break
    if len(rows) > 8:
        surf.blit(fonts["ui_small"].render("Further products are in the Products tab.", True, DIM), (rect.x, y))
        y += 22
    surf.blit(fonts["section"].render("RADIATION", True, ACCENT), (rect.x, y + 6))
    y += 28
...
def _frame(surf, fonts, rect, title, subtitle) -> None:
    pygame.draw.rect(surf, SURFACE_2, rect, border_radius=10)
    pygame.draw.rect(surf, HAIRLINE, rect, 1, border_radius=10)
    surf.blit(fonts["ui_small"].render(title, True, TEXT), (rect.x + 12, rect.y + 6))
    surf.blit(fonts["mono_small"].render(subtitle, True, DIM), (rect.x + 12, rect.y + 26))


def _plot_rect(rect: pygame.Rect) -> pygame.Rect:
    return pygame.Rect(rect.x + 12, rect.y + 48, rect.w - 24, rect.h - 68)
