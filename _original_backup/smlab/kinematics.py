...
def isr_fraction(uniform: float, beta: float) -> float:
    """Map a uniform (0, 1] deviate to the energy fraction v of one collinear photon.

    The Kuraev–Fadin leading-log measure satisfies
    ∫ β v^{β-1} dv = du after the substitution v = u^{1/β}.
    """
    u = min(max(uniform, 1.0e-15), 1.0)
    return u ** (1.0 / beta)
