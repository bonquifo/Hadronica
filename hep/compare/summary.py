...
def madgraph_table() -> dict:
    ...
    afb = [(r["smlab_matched_afb"] - r["madgraph_afb"]) / r["madgraph_afb_err"] for r in rows if "madgraph_afb" in r]
...
def generator_table() -> list[tuple[str, dict]]:
    smlab = json.load(open(os.path.join(ROOT, "hep", "validation", "results.json"), encoding="utf-8"))["benchmarks"]
    ...
    for key in ("lep_z_hadrons", "lhc_minbias", "lhc_z_pt", "lhc_ttbar", "lhc_ttbar_dilep", "lhc_jets"):
        ...
        table.append((key, {
            "smlab_lo": smlab[key]["median_chi2_ndf"],
            "smlab_best": smlab[best_key]["median_chi2_ndf"],
            "smlab_best_variant": smlab[best_key]["variant"],
...
def main() -> None:
    ...
    for key, row in generator_table():
        ...
        print(f"{key:17s} {row['smlab_lo']:9.2f} {fmt(row['pythia_direct'])} {fmt(row['herwig'])} "
              f"{fmt(row['sherpa'])}   {row['smlab_best']:.2f} ({row['smlab_best_variant']})")
