...
# PDG widths, GeV. The Higgs entry is the SM prediction quoted by the PDG.
_WIDTH_GEV = {
    6: GAMMA_T,
    23: GAMMA_Z,
    24: GAMMA_W,
...
def english_name(pdg: int) -> str:
    ...
    return _ENGLISH.get(pdg, species(pdg).name)
...
def report_cards(event) -> list[tuple[str, tuple[str, ...]]]:
    ...
    if len(products) > 2:
        cards.append(("Also", ("More was produced", "Open More, then Every product")))
    return cards
...
def _carrier(particle) -> EnergyCarrier:
    ...
    rest = 0.0 if abs(particle.pdg) == 22 else species(particle.pdg).mass
...
def brief_result(particle) -> str:
    ...
    if proper is None or spec.mass <= 0.0:
        return f"{energy}   stable"
    if particle.status == "intermediate":
        return f"{energy}   decays at the collision"
    flight = (particle.p4.p / spec.mass) * proper * C_M_PER_S
    if flight >= 10.0:
...
def _where(pdg: int) -> str:
    ...
    if kind == 21 or is_quark(pdg) or kind in (111, 211):
...
def generated_summary(event) -> str:
    ...
    note = ""
    if higgs:
        note = " The Higgs mean life uses the Standard Model width 4.07 MeV quoted by the PDG, not a measured width."
    return f"{head} {body}{note}"


def _clause(particle) -> str:
    name = "γ ISR" if particle.status == "isr" else species(particle.pdg).name
    energy = format_energy(particle.p4.e)
    kind = lifetime_kind(particle.pdg)
    if kind == "parton":
    ...
    spec = species(particle.pdg)
    proper = proper_lifetime_s(particle.pdg)
    if spec.mass <= 0.0 or proper is None:
        return f"{name} {energy}, stable."
    lab = (particle.p4.e / spec.mass) * proper
    flight = (particle.p4.p / spec.mass) * proper * C_M_PER_S
    fate = "decayed in this event" if particle.status == "intermediate" else "left undecayed"
    return (
