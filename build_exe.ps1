...
$data = @(
    ...
    "hep\ext\hadronica_fxfx.cpp;hep\ext", "hep\ext\build_fxfx.sh;hep\ext",
    "hadronica\assets\window_icon.png;hadronica\assets"
)
...
$pyiArgs = @("--noconfirm", "--clean", "--windowed", "--onefile", "--name", "Hadronica", "--collect-all", "pygame",
             "--icon", "hadronica\assets\icon.ico")
if ($env:HADRONICA_DIST) { $pyiArgs += @("--distpath", $env:HADRONICA_DIST) }
...
python -m PyInstaller @pyiArgs run_hadronica.py 2>&1 | ForEach-Object { "$_" }
