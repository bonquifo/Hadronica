...
on quark channels). Higgs branching fractions are the Standard Model table of
the LHC Higgs Cross Section Working Group (CERN Yellow Report 4,
CERN-2017-002-M) at m_H = 125.10 GeV, the tabulated mass closest to the PDG 2026
value 125.13 GeV; the nine listed modes sum to 0.9997 and are renormalized to one.

Tau decays use PDG exclusive fractions for the leptonic, one-pion, rho, a1,
and three-prong-plus-π⁰ channels. The remaining few percent (kaon modes,
π⁻3π⁰, and higher multiplicity) is represented by a charge-conserving
one-prong four-pion state so 4-momentum and charge stay exact.

Leptonic muon and tau decays use the massless Michel spectrum (L. Michel,
Proc. Phys. Soc. A 63, 514 (1950)). Decay angles of
...
propagators in q² times two-body phase space and the H → VV matrix element.
"""
...
# PDG 2026 tau branching fractions (fractions, not percent). The last entry is
# one minus the listed modes: kaon modes, π⁻3π⁰ν (1.04%), and higher multiplicity.
TAU_BR: tuple[tuple[str, float], ...] = (
    ("enu", 0.1785),
    ("munu", 0.1737),
    ("pi", 0.1082),
    ("rho", 0.2549),  # τ⁻ → π⁻ π⁰ ν
    ("a1_neutral", 0.0926),  # τ⁻ → π⁻ 2π⁰ ν (ex. K⁰), via a1 → ρ⁻ π⁰
    ("a1_charged", 0.0899),  # τ⁻ → π⁻ π⁺ π⁻ ν (ex. K⁰, ω), via a1 → ρ⁰ π⁻
    ("three_prong_pi0", 0.0462),  # τ⁻ → π⁻ π⁺ π⁻ π⁰ ν
    ("rest", 0.0560),
)
...
def w_channel_weights(mass: float) -> list[tuple[float, int, int]]:
    ...
    for (up, down), v_abs in CKM_ABS.items():
        channels.append((up, -down, 3.0 * qcd * v_abs * v_abs))
    out: list[tuple[float, int, int]] = []
    for pdg_a, pdg_b, weight in channels:
        m1 = species(pdg_a).mass
        m2 = species(pdg_b).mass
        if mass <= m1 + m2:
            continue
        # Massless V-A weight (1 per lepton, 3|V|²(1+α_s/π) per quark pair)
        # times the exact tree-level mass factor, so heavy channels close smoothly.
        out.append((weight * _va_mass_factor(mass, m1, m2), pdg_a, pdg_b))
    return out


def _va_mass_factor(mass: float, m1: float, m2: float) -> float:
    """Γ(m1, m2) / Γ(0, 0) for a vector boson decaying through a V-A current.

    (2p/M) [1 - (m1² + m2²)/(2M²) - (m1² - m2²)²/(2M⁴)], which is 1 when both
    daughters are massless and 0 at threshold.
    """
    lam = kallen(mass * mass, m1 * m1, m2 * m2)
    if lam <= 0.0:
        return 0.0
    p = math.sqrt(lam) / (2.0 * mass)
    m2_sum = (m1 * m1 + m2 * m2) / (mass * mass)
    m2_diff = (m1 * m1 - m2 * m2) / (mass * mass)
    return (2.0 * p / mass) * (1.0 - 0.5 * m2_sum - 0.5 * m2_diff * m2_diff)
...
def sample_virtual_pair(
    ...
    The joint density in (q1², q2²) is a product of Breit–Wigners, two-body
    phase space λ^½, and the H → VV matrix element (λ + 12 q1² q2²/M_H⁴)
    [Djouadi, Phys. Rept. 457 (2008) 1]. The grid is uniform in mass, so the
    Jacobian dq1² dq2² = 4 m1 m2 dm1 dm2 is included. The returned masses are
    spread uniformly inside the chosen grid cell.
    """
    ...
    if cached is None:
        ...
        jacobian = m1 * m2
        weight = np.where(ok, bw * ps * jacobian, 0.0).ravel()
    ...
    step = float(masses[1] - masses[0])
    for _attempt in range(64):
        u = float(rng.random()) * total
        index = int(np.searchsorted(cdf, u, side="left"))
        index = min(index, cdf.size - 1)
        i, j = divmod(index, masses.size)
        a = float(masses[i]) + (float(rng.random()) - 0.5) * step
        b = float(masses[j]) + (float(rng.random()) - 0.5) * step
        if a >= m_min and b >= m_min and a + b < m_parent - 0.05:
            return a, b
...
def _decay_tau_channel(
    ...
    if channel == "three_prong_pi0":
        # τ⁻ → π⁻ π⁻ π⁺ π⁰ ν: three charged tracks plus a π⁰, phase space only.
        pions = [-211 * sign, -211 * sign, 211 * sign, 111]
        return _tau_with_pions(parent, nu, pions, rng)
    if channel == "rest":
        # τ⁻ → π⁻ π⁰ π⁰ π⁰ ν, a one-prong stand-in for the kaon and other rare modes
        # that keeps charge and 4-momentum exact.
        pions = [-211 * sign, 111, 111, 111]
        return _tau_with_pions(parent, nu, pions, rng)
...
def _tau_a1(
    ...
    # a1 mass must leave room for a neutrino and for the three pions inside the a1.
    m_pi = species(211).mass
    m_min = 3.0 * m_pi + 1.0e-3
    ...
    if rho_pdg == 113:
        m_rho = _sample_resonance_mass(M_RHO0, GAMMA_RHO0, m_rho_min, m_rho_max, rng)
    else:
