#!/usr/bin/env bash
# Build the smlab_fxfx Python module (hep/ext/smlab_fxfx.cpp) into the smlab-hep env.
set -euo pipefail
HERE=$(cd "$(dirname "$0")" && pwd)
E=$HOME/micromamba/envs/smlab-hep
...
"$E/bin/x86_64-conda-linux-gnu-g++" -O2 -shared -fPIC -std=c++17 \
    ...
    "$HERE/smlab_fxfx.cpp" -L"$E/lib" -lpythia8 -Wl,-rpath,"$E/lib" \
    -o "$SITE/smlab_fxfx$SUFFIX"
"$E/bin/python" -c "import os, sys, pythia8, smlab_fxfx; p = pythia8.Pythia(os.path.join(sys.prefix, 'share', 'Pythia8', 'xmldoc'), False); smlab_fxfx.attach(p); print('smlab_fxfx ok')"
