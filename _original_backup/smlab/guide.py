...
def focus_card(action: str | None, payload, ctx: GuideContext) -> GuideCard:
    if action is None:
        return GuideCard(
            ...
            "This side says what that piece is and what it changes in the collision on the left. "
            "Custom, next to the particle pairs, chooses both incoming particles, each momentum, and the angle between them. "
            "F1 opens the formulas and the list of approximations. "
    ...
    if action == "methods":
        return GuideCard(
            "Methods",
            "Opens the formulas, the PDG 2024 inputs, and what this Born laboratory leaves out: "
...
def _toggle_card(field: str, ctx: GuideContext) -> GuideCard:
    if field == "isr":
        ...
        return GuideCard(
            ...
            "One collinear photon is sampled from the Kuraev–Fadin radiator and the hard process runs at "
...
def _shape_body(ctx: GuideContext) -> str:
    return (
        "Blue is the Born cross section of the selected process. Gold includes the leading-log radiator "
