...
def test_fermion_pair_matches_breit_wigner_and_low_energy_qed():
    ...
    photon = (4.0 * math.pi * ALPHA * ALPHA / (3.0 * M_Z**2)) * (
    ...
    # Classic result: σ(e+e- → μ+μ-) ≈ (86.8 nb GeV²) / s well below the Z.
    sqrt_s = 10.0
    qed = (4.0 * math.pi * ALPHA * ALPHA / (3.0 * sqrt_s**2)) * to_pb(1.0) / 1000.0
    full_nb = to_pb(fermion_pair_sigma(sqrt_s, 11, 13)) / 1000.0
    # qed above is σ in nb for the massless point cross section at this s.
    assert sqrt_s**2 * qed == pytest.approx(86.8, rel=0.005)
    assert full_nb == pytest.approx(qed, rel=0.005)
...
def test_w_leptonic_branching_fraction_is_near_the_pdg():
    ...
    # PDG is about 10.8% per leptonic species after the QCD enhancement of quarks.
    assert electron == pytest.approx(0.108, abs=0.005)
