...
from smlab.constants import ALPHA, PB_PER_GEV2
...
def convolute_born(
    ...
    """Leading-log collinear ISR convolution of a Born cross section, in GeV^-2.

    The substitution v = u^{1/β} turns the Kuraev–Fadin measure into a flat
    integral over u, so the convolved cross section is the average of σ(s'(u)).
    """
    beta = isr_exponent(sqrt_s, beam_mass, ALPHA)
    total = 0.0
    for i in range(n):
        u = (i + 0.5) / n
        v = isr_fraction(u, beta)
        if v >= 1.0:
            continue
...
def _sample_isr_shat(born, sqrt_s: float, beam_mass: float, rng: np.random.Generator) -> tuple[float, float]:
    """Sample s' from the ISR-convolved Born cross section on a flat-u grid."""
    beta = isr_exponent(sqrt_s, beam_mass, ALPHA)
    n = 640
    weights = np.empty(n, dtype=float)
    shats = np.empty(n, dtype=float)
    ...
    index = min(index, n - 1)
    return float(shats[index]), float(vs[index])
...
def generate_event(
    ...
    *,
    ...
    born = lambda shat: process.born_sigma(beam, shat)

    if use_isr:
        shat, v = _sample_isr_shat(born, sqrt_s, species(beam.pdg_plus).mass, rng)
...
def sigma_pb(process: Process, beam: BeamMode | str, sqrt_s: float, isr: bool) -> float:
    """Cross section in picobarns, optionally convolved with leading-log ISR."""
    if isinstance(beam, str):
        beam = BEAMS[beam]
    if not process.allowed(beam, sqrt_s):
        return 0.0
    ...
    if isr and beam.id in ("ee", "mumu"):
        value = convolute_born(born, sqrt_s, species(beam.pdg_plus).mass)
    else:
        value = born(sqrt_s)
